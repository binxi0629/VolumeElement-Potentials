...
class SymEnergyDataset(Dataset):
    ...
    def normalize_X(self, mode='standard', pre_as_mean=None, pre_as_std=None, pre_as_max=None, pre_as_min=None,
                    pre_rs_mean=None, pre_rs_std=None, pre_rs_max=None, pre_rs_min=None):
        if mode == "standard":
            self.min_shift = 1e-5
            ...
            self.min_shift = 1e-5
            self.asym_df = (self.asym_df - self.asym_df.min()) / (
                        self.asym_df.max() - self.asym_df.min()) + self.min_shift
            self.rsym_df = (self.rsym_df - self.rsym_df.min()) / (
                        self.rsym_df.max() - self.rsym_df.min()) + self.min_shift
        elif mode == "minmax_allaxis":
            self.asym_df = (self.asym_df - self.asym_df.min().min()) / (
                        self.asym_df.max().max() - self.asym_df.min().min()) + self.min_shift
            self.rsym_df = (self.rsym_df - self.rsym_df.min().min()) / (
                        self.rsym_df.max().max() - self.rsym_df.min().min()) + self.min_shift
        else:
...
class DPNN(Dataset):
    def __init__(self, f_df, label_df, n_col, n_ve=60, shuffle_X=False):
...
class VEStressDataset(Dataset):
    def __init__(self, df, ref_x, ref_sub_e, xs_cols=['x1', 'y1', 'x3', 'y3', 'x5', 'y5'], forces_cols=['fx', 'fy'],
                 sub_e_cols=['sub_e']):
        # FIXME: try all vertices
        xs_cols = ['x0', 'y0', 'x1', 'y1', 'x2', 'y2', 'x3', 'y3', 'x4', 'y4', 'x5', 'y5']

        self.xs_df = df[xs_cols]
        self.forces_df = df[forces_cols]
        self.sub_e_df = df[sub_e_cols]
        # Getting dx from equil pos and elastic energy

        # FIXME: uncomment the following line
        # self.xs_df -= ref_x
        self.sub_e_df -= ref_sub_e

        self.length = len(self.xs_df)
        return

    def normalize_X(self, mode='standard', pre_mean=None, pre_std=None, pre_max=None, pre_min=None):
        if mode == "standard":
            self.min_shift = 1e-5
            self.xs_mean = pre_mean if pre_mean is not None else self.xs_df.mean()
            self.xs_std = pre_std if pre_std is not None else self.xs_df.std()
            self.xs_df = (self.xs_df - self.xs_mean) / self.xs_std + self.min_shift
        elif mode == "minmax":
            self.min_shift = 1e-5
            self.xs_max = pre_max if pre_max is not None else self.xs_df.max()
            self.xs_min = pre_min if pre_min is not None else self.xs_df.min()
            self.xs_df = (self.xs_df - self.xs_min) / (self.xs_max - self.xs_min) + self.min_shift
        else:
            print("Unrecognized method, not normalization applied")

    def __getitem__(self, index):
        return self.xs_df.iloc[index].values, self.forces_df.iloc[index].values, self.sub_e_df.iloc[index].values
...
#     '''
#     For generating angular and radial symmetry functions outputs into csv files
#     '''
#     sroot = "AE_root"
#     spath = [sroot+"/"+dir+"/POSCAR" for dir in os.listdir(sroot) if 'perturb' in dir]
#     lpath = "AreaElement.json"
#     ds = CoordsEnergyDataset(spath, lpath)
#     rsym, asym = [], []
#     for i, (X, y) in enumerate(ds):
#         rsym.append(radial_sym(pairwise_dist(X), 10))
#         asym.append(angular_sym(pairwise_vec(X), pairwise_dist(X)))
#     rsym = np.array(rsym)
#     asym = np.array(asym)
#     rsym_df = pd.DataFrame(rsym, columns=[f'rsym_{i}' for i in range(60)])
#     asym_df = pd.DataFrame(asym, columns=[f'asym_{i}' for i in range(60)])
#     rsym_df.to_csv('rsym.csv', float_format='%.6f')
#     asym_df.to_csv('asym.csv', float_format='%.6f')